...
while ! curl -s http://ollama:11434/api/show -d '{"name":"llama3.1:8b"}' | grep -q "llama3.1:8b"; do
    if [ $RETRY_COUNT -eq 0 ]; then
        echo "Model not found, pulling llama3.1:8b..."
        # Pull with progress
        curl -X POST http://ollama:11434/api/pull -d '{"name": "llama3.1:8b"}' | while read -r line; do
            echo "$line"
