...
RUN pip install -U openai-whisper faster-whisper && \
    ...
    python3 -c 'from huggingface_hub import snapshot_download; snapshot_download("Systran/faster-whisper-base.en", local_dir="/app/models/whisper")' && \
...
RUN echo '#!/bin/bash' > /app/docker-entrypoint.sh && \
    ...
    echo '  python3 -c "from huggingface_hub import snapshot_download; snapshot_download(\"Systran/faster-whisper-base.en\", local_dir=\"/app/models/whisper\")"' >> /app/docker-entrypoint.sh && \
