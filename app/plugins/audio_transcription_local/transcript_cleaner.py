"""Utility module for cleaning up transcripts by removing filler words and repetitions."""
...
class TranscriptCleaner:
    def __init__(self):
        ...
        self.context_dependent_words = {"like", "so", "well", "right"}
    ...
    def remove_filler_words(self, text: str) -> str:
        ...
        pattern = (
            r"[,\s]*\b("
            + "|".join(re.escape(word) for word in self.filler_words)
            + r")\b[,\s]*"
        )
            ...
            while i < len(words):
                ...
                for phrase_length in range(4, 0, -1):
                    if i + phrase_length * 2 <= len(words):
                        ...
                        phrase2 = " ".join(
                            words[i + phrase_length : i + phrase_length * 2]
                        )
