...
class NoiseReductionPlugin(PluginBase):
    ...
    def __init__(
        self, config: PluginConfig, event_bus: EventBus | None = None
    ) -> None:
    ...
    self, mic_data: np.ndarray, sys_data: np.ndarray, sample_rate: int
        ...
        mic_aligned = np.pad(mic_aligned, (0, max_len - len(mic_aligned)), mode="constant")
        sys_aligned = np.pad(sys_aligned, (0, max_len - len(sys_aligned)), mode="constant")
        ...
        try:
            logger.info(
                ...
                extra={
                    ...
                    "event_id": event_data.event_id if hasattr(event_data, "event_id") else event_data.get("event_id"),
            ...
            if isinstance(event_data, dict):
                ...
                metadata = event_data.metadata if hasattr(event_data, "metadata") else {}
            ...
            source_plugin = (
                event_data.get("source_plugin") if isinstance(event_data, dict)
                else getattr(event_data.context, "source_plugin", None) if hasattr(event_data, "context")
                else None
            )
